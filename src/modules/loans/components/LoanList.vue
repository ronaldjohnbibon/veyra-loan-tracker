  ...
  <ion-list v-else class="record-list" lines="full">
    <ion-item v-for="loan in loans" :key="loan.id" :router-link="`/loans/${loan.id}`">
      <ion-label>
        ...
        <p>
          Principal {{ formatCurrency(fromCents(loan.principalCents)) }} | Total
          {{ formatCurrency(fromCents(loan.totalDueCents)) }}
        </p>
        <p>
          Paid {{ formatCurrency(fromCents(loan.paidCents)) }} | Remaining
          {{ formatCurrency(fromCents(loan.remainingCents)) }}
        </p>
